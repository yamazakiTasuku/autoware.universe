...
public:
  PubManager() : Node("test_pub_node")
  {
    ...
    pub_odo_ =
      create_publisher<Odometry>("/behavior_velocity_planner_node/input/vehicle_odometry", 1);
    pub_acc_ = create_publisher<AccelWithCovarianceStamped>(
      "/behavior_velocity_planner_node/input/accel", 1);
    pub_map_ = create_publisher<HADMapBin>("/behavior_velocity_planner_node/input/vector_map", 1);
    ...
    pub_occ_ =
      create_publisher<OccupancyGrid>("/behavior_velocity_planner_node/input/occupancy_grid", 1);
  }
  ...
  void publishHADMapBin()
  {
    ...
    const auto lanelet2_map_projector_type =
      declare_parameter("lanelet2_map_projector_type", "MGRS");
